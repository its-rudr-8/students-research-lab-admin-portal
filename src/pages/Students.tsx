...
export default function Students(){
  ...
  const handleEdit=async()=>{if(!editSt||!canEdit)return;try{const r=await adminAPI.updateStudent(editSt.id!,form);if(r.success&&r.data){setStudents(p=>p.map(s=>s.id===editSt.id?r.data:s));setEditSt(null);toast({title:"Student updated"});}}catch(e:any){toast({variant:"destructive",title:"Error",description:e.message});}};
  const handleDel=async(s:Student)=>{if(!canEdit)return;try{await adminAPI.deleteStudent(s.id!);setStudents(p=>p.filter(x=>x.id!==s.id));setSel(prev=>{const n=new Set(prev);n.delete(s.enrollment_no);return n;});toast({title:"Student removed"});}catch(e:any){toast({variant:"destructive",title:"Error",description:e.message});}};
  const handleBulkDel=async()=>{if(!canEdit)return;const rm=students.filter(s=>sel.has(s.enrollment_no));try{await Promise.all(rm.map(s=>adminAPI.deleteStudent(s.id!)));setStudents(p=>p.filter(s=>!sel.has(s.enrollment_no)));setSel(new Set());toast({title:`${rm.length} students removed`});}catch(e:any){toast({variant:"destructive",title:"Error",description:e.message});}};
            ...
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              <thead>
                <tr style={{background:"linear-gradient(90deg,#f8f6f1,#fbfaf7)"}}>
                  ...
                  {THcols.map((col,ci)=>(
                    <th key={col} className={THcls[ci]} style={{padding:"13px 16px",textAlign:(ci===5?"center":"left") as const,fontSize:"0.72rem",fontWeight:800,letterSpacing:"0.12em",textTransform:"uppercase" as const,color:"#6a6050",borderBottom:"2px solid #ede8e0",whiteSpace:"nowrap" as const}}>{col}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                        ...
                        <td style={{padding:"12px 16px",textAlign:"center" as const}}>
                          {canEdit&&(
                            <div style={{display:"inline-flex",gap:6,alignItems:"center"}}>
                              <button title="Edit" onClick={()=>openEdit(s)} style={{width:30,height:30,borderRadius:8,border:"1.5px solid #c8d8c0",background:"#edf6f0",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",transition:"all 0.15s"}} onMouseEnter={e=>(e.currentTarget.style.background="#d4ecdc")} onMouseLeave={e=>(e.currentTarget.style.background="#edf6f0")}><Pencil size={13} color="#1e5c42"/></button>
                              <button title="Deactivate" onClick={()=>handleDel(s)} style={{width:30,height:30,borderRadius:8,border:"1.5px solid #f4c0c4",background:"#fdedf0",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",transition:"all 0.15s"}} onMouseEnter={e=>(e.currentTarget.style.background="#f8d0d4")} onMouseLeave={e=>(e.currentTarget.style.background="#fdedf0")}><Trash2 size={13} color="#c0363a"/></button>
                            </div>
                          )}
                        </td>
                      </motion.tr>
